...
| **CDAE** | 사용자 행동 데이터를 반영한 협업 필터링 기반 추천 |

### 🛠️ 모델 학습 과정

#### ✅ FastText 기반 단어 임베딩 학습
- **학습 데이터**: 한국어 Wikipedia 데이터
- **학습 과정**:
  1. 텍스트 데이터를 정제하고 토큰화 진행
  2. FastText 모델(`vector_size=100`, `window=5`, `min_count=5`)로 임베딩 학습
  3. 학습된 임베딩을 활용하여 상품명을 벡터화

#### ✅ K-Means 클러스터링 기반 추천
- **목적**: 상품 간 유사도를 기반으로 그룹화하여 추천
- **학습 과정**:
  1. FastText 임베딩을 이용하여 상품 벡터 생성
  2. K-Means(`n_clusters=20`)을 사용하여 상품 군집화
  3. 동일한 클러스터 내에서 유사 상품 추천

#### ✅ CDAE (Contractive Denoising Autoencoder) 협업 필터링
- **목적**: 사용자 행동 데이터(검색, 클릭, 즐겨찾기)를 반영한 추천
- **학습 과정**:
  1. 사용자-상품 상호작용 데이터를 기반으로 희소 행렬 생성
  2. CDAE 모델을 설계하여 사용자 임베딩과 상품 임베딩을 결합
  3. 학습된 모델을 활용해 개인화된 추천 제공

### 📊 모델 평가 및 성능 비교

| 평가 지표 | 설명 | CDAE 성능 |
|----------|--------------------------------------|------------|
| **Precision** | 추천된 상품 중 사용자가 실제 관심을 가진 비율 | **0.6812** |
| **Recall** | 실제 관심 상품 중 추천된 상품의 비율 | **0.9960** |
| **NDCG@10** | 추천된 상품의 순위가 적절한지를 평가 | **0.9966** |

---

## 4. [웹 프로젝트: 홈쇼핑 데이터 기반 추천 시스템](https://github.com/yusongod0303/WEB_Deployment)
